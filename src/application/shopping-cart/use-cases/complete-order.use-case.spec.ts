...
const product = new Product('p1', 0, 'phone', '', 'iPhone', '', 999, 0, '', '', '');
const cartWithItems = new ShoppingCart('cart-1', [{ count: 1, item: product }], { price: 999, discount: 0, finalPrice: 999 });
const emptyCart = new ShoppingCart('cart-1', [], { price: 0, discount: 0, finalPrice: 0 });
...
describe('CompleteOrderUseCase', () => {
  ...
  it('completes order and clears cart from user', async () => {
    ...
    mockUserRepo.updateShoppingCart.mockResolvedValue({ ...mockUser, shoppingCartId: null });
