...
const mockUser = new User('user-1', 'John', 'Doe', 30, 'john@test.com', 'pass', null);
const mockCart = new ShoppingCart('cart-uuid', [], { price: 0, discount: 0, finalPrice: 0 });

describe('CreateShoppingCartUseCase', () => {
  let useCase: CreateShoppingCartUseCase;
  const mockCartRepo = { create: jest.fn() };
  const mockUserRepo = { findByEmail: jest.fn(), updateShoppingCart: jest.fn() };
  ...
  it('creates cart and links it to user', async () => {
    ...
    mockUserRepo.updateShoppingCart.mockResolvedValue({ ...mockUser, shoppingCartId: 'cart-uuid' });

    const result = await useCase.execute('john@test.com');
    expect(result).toBe(mockCart);
    expect(mockUserRepo.updateShoppingCart).toHaveBeenCalledWith('user-1', 'cart-uuid');
  });
});
