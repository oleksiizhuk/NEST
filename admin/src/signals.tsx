...
export const RULES: Record<SignalRule, { title: string; hint: string }> = {
  ...
  overload: {
    title: 'Перегружен',
    hint: 'Не давайте новых задач, пока очередь не уменьшится; обсудите, что из очереди отдать тому, у кого меньше.',
  },
  underload: {
    title: 'Недогружен',
    hint: 'Предложите свободную задачу из релиза или помощь коллеге. Иногда человек занят тем, чего нет в Jira, — спросите.',
  },
  runway: {
    title: 'Скоро нечего делать',
    hint: 'Заранее договоритесь о следующей задаче, чтобы не было простоя. Лучше взять релизную и приоритетную.',
  },
  switching: {
    title: 'Много областей сразу',
    hint: 'Каждое переключение стоит времени. Помогите сузить фокус: одна-две области, остальное — другим.',
  },
  unplanned: {
    title: 'Много незапланированного',
    hint: 'Если баги всегда идут одному человеку, он перестаёт делать плановую работу. Распределите дежурство по багам.',
  },
